...
wss.on('request', ws => {
    ...
    connection.on('message', raw => {
        let message = JSON.parse(raw.utf8Data);
        let adapter = new FileSync('data/' + message.game_id + '.json');
        let db = low(adapter);
        const deal = () => {
            let this_deck = _.shuffle(_.clone(deck));
        ...
        Object.keys(db.get('private').value())
            ...
            if (client) {
                const response = Object.assign(Object.assign({}, db.get(['private', user_id]).value()), db.get('shared').value());
